...
WALLET_ADDRESS=$(docker logs nosana-node | head -n 14 | grep -i -m 1 "Wallet" | cut -c 17- | tr -cd '[:alnum:]' | sed 's/0m$//')
#JOB_ADDRESS=$(docker logs --tail 150 nosana-node | tac | grep -m 1 "Claimed job" | awk -F'Claimed job ' '{print "https://explorer.nosana.io/jobs/" $2}')

# Job Search Timeout in Seconds
JobSearchTimeout=0

# Run the command with a timeout and capture the output
JOB_ADDRESS=$(timeout $JobSearchTimeout bash -c 'docker logs --tail 150 nosana-node | tac | grep -m 1 "Claimed job" | awk -F"Claimed job " "{print \"https://explorer.nosana.io/jobs/\"\$2}"')
